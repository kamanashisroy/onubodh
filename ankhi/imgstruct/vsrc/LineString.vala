...
public class onubodh.LineString : StringStructureImpl {
	SearchableOPPFactory<ImageMatrixStringNearLinearPlus> memory;
	public LineString(netpbmg*src, aroop_uword8 minGrayVal, int radius_shift, int[] featureVals, int[] featureOps) {
		...
		memory = SearchableOPPFactory<ImageMatrixStringNearLinearPlus>.for_type(128,factory_flags.SWEEP_ON_UNREF | factory_flags.EXTENDED | factory_flags.SEARCHABLE | factory_flags.MEMORY_CLEAN);
	}
	~LineString() {
		memory.destroy();
	}
